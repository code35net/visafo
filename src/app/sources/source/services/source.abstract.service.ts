...
import type { GetSourcesInput, SourceDto } from '../../../proxy/sources/models';
import { SourceService } from '../../../proxy/sources/source.service';

export abstract class AbstractSourceViewService {
  protected readonly proxyService = inject(SourceService);
  ...
  data: PagedResultDto<SourceDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<SourceDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetSourcesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<SourceDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetSourcesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'Source.xlsx');
      });
  }
}
