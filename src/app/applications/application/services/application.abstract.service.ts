...
import type { GetApplicationsInput, ApplicationDto } from '../../../proxy/applications/models';
...
export abstract class AbstractApplicationViewService {
  ...
  data: PagedResultDto<ApplicationDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<ApplicationDto[]>([]);
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<ApplicationDto>) => {
      this.data = list;
