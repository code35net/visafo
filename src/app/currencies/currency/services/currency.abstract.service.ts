...
import type { GetCurrenciesInput, CurrencyDto } from '../../../proxy/currencies/models';
import { CurrencyService } from '../../../proxy/currencies/currency.service';

export abstract class AbstractCurrencyViewService {
  protected readonly proxyService = inject(CurrencyService);
  ...
  data: PagedResultDto<CurrencyDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<CurrencyDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetCurrenciesInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<CurrencyDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetCurrenciesInput;
    this.list.get();
  }
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'Currency.xlsx');
      });
  }
}
