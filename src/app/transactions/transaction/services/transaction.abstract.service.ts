...
export abstract class AbstractTransactionViewService {
  protected readonly proxyService = inject(TransactionService);
  ...
  protected bulkDeleteRequest() {
    const ids = this.selected().map(({ transaction: { id } }) => id);
  ...
  delete(record: TransactionWithNavigationPropertiesDto) {
    this.confirmationService
      ...
      .pipe(
        ...
        switchMap(() => this.proxyService.delete(record.transaction.id)),
      )
  ...
  exportToExcel() {
    ...
    this.proxyService
      ...
      .subscribe(result => {
        this.abpWindowService.downloadBlob(result, 'Transaction.xlsx');
      });
  }
}
